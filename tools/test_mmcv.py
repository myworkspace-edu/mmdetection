...
def parse_args():
    ...
    parser.add_argument(
        '--img', default='../testdata', help='Image path, include image file, dir and URL.')
    parser.add_argument('--config', default='../configs/faster_rcnn/faster-rcnn_r101_fpn_2x_coco.py', help='Config file')
    parser.add_argument('--checkpoint', default='../checkpoints/faster_rcnn_r101_fpn_2x_coco_bbox_mAP-0.398_20200504_210455-1d2dac9c.pth', help='Checkpoint file')
    parser.add_argument(
...
def main():
    ...
    _, _, files = os.walk(args.img)
    # start detector inference
    print(f'Performing inference on {len(files)} images.... '
          'This may take a while.')
    # progress_bar = ProgressBar(len(files))

    for path, folders, files in os.walk(args.img):
        for file in files:
